...
+ (void)displayActionPickerWithView:(UIView *)aView data:(NSArray *)data selectedIndex:(NSInteger)selectedIndex target:(id)target action:(SEL)action title:(NSString *)title clientData:(id)clientData {
	ActionSheetPicker *actionSheetPicker = [[ActionSheetPicker alloc] initForDataWithContainingView:aView data:data selectedIndex:selectedIndex target:target action:action title:title clientData:clientData];
	[actionSheetPicker showActionPicker];
	[actionSheetPicker release];
}

+ (void)displayActionPickerWithView:(UIView *)aView datePickerMode:(UIDatePickerMode)datePickerMode selectedDate:(NSDate *)selectedDate target:(id)target action:(SEL)action title:(NSString *)title clientData:(id)clientData {
	ActionSheetPicker *actionSheetPicker = [[ActionSheetPicker alloc] initForDateWithContainingView:aView datePickerMode:datePickerMode selectedDate:selectedDate target:target action:action title:title clientData:clientData];
	[actionSheetPicker showActionPicker];
	[actionSheetPicker release];
}
...
- (id)initForDataWithContainingView:(UIView *)aView data:(NSArray *)data selectedIndex:(NSInteger)selectedIndex target:(id)target action:(SEL)action title:(NSString *)title clientData:(id) clientData {
...
- (id)initForDateWithContainingView:(UIView *)aView datePickerMode:(UIDatePickerMode)datePickerMode selectedDate:(NSDate *)selectedDate target:(id)target action:(SEL)action title:(NSString *)title clientData:(id) clientData {
	if ([self initWithContainingView:aView target:target action:action] != nil) {
        self.clientData = clientData;
...
- (void)showActionPicker {
    ...
    pickerDateToolbar.tintColor = [UIColor grayColor];
...
- (void)actionPickerDone {
	...
	if (nil != self.data) {
		...
		[self.target performSelector:self.action withObject:[NSNumber numberWithInt:self.selectedIndex] withObject:self.clientData];
	} else {
		//send date picker message
		[self.target performSelector:self.action withObject:self.selectedDate withObject:self.clientData];
	}
